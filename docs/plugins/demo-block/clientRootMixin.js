...
// var Vue$ = _interopDefault(require('vue/dist/vue.esm.js'));
import Vue$ from 'vue'
...
var CLASS_FOOTER_FIX = 'vuepress-plugin-demo-block__footer-fix'
...
var getVueScript = function getVueScript(result) {
  ...
  const returnIndex = scriptStr.indexOf('return') + 7
  let moduleScriptString = scriptStr.slice(returnIndex)

  const endIndex = moduleScriptString.lastIndexOf('};')
  moduleScriptString = moduleScriptString.slice(0, endIndex + 1)
  moduleScriptString = `export default ${moduleScriptString}`

  
  var scriptObj = [eval][0](scriptStr)
  scriptObj.template = html
  result.js = moduleScriptString
  return scriptObj
}
...
var getVueDetail = function getVueDetail(code, config) {
  ...
  result.script = getVueScript(result)
...
function getVuepBtn(code, ctx) {  
  var link = h('a', {
    style: 'position:absolute; top: 0; height:100%; right: 20px; line-height: 42px; font-size: 14px',
    ...
    onclick: () => {
      ...
      const content = { code, ui }
      localStorage.setItem(`playground_content_${id}`, JSON.stringify(content))
      const { href } = ctx.$router.resolve(`/demo/playground.html?id=${id}`)
...
function getCodepenBtn(_ref) {
  var css = _ref.css
  var htmlTpl = _ref.htmlTpl
  var jsTpl = _ref.jsTpl
  var jsLib = _ref.jsLib
  var cssLib = _ref.cssLib
...
function webController(ctx) {
  ...
  nodes.forEach(function (node) {
    ...
    var code = decodeURIComponent(window['DEMO_BLOCK_MAP_' + node.dataset.codeIndex])
    ...
    // }
    // 
    footerNode.appendChild(getVuepBtn(code, ctx))
    ...
    if (type === 'react') {
      ...
      // console.log('Vue === window.Vue', Vue === window.Vue)
...
function expandHandler(expandNode, height, codeNode, footerNode) {
  ...
  const $scrollHandler = function() { scrollHandler(footerNode, codeNode) }
  ...
  if (!isExpand) {
    footerNode.classList.remove(CLASS_FOOTER_FIX)
    footerNode.style.left = '0';
    footerNode.style.width = 'auto';
    footerNode.dataset.fixed = '0';
    removeScrollHandler($scrollHandler)
    return;
  }
  // 展开动画结束之后再计算位置
  setTimeout(() => {
    scrollRoot ? null : scrollRoot = document
    scrollRoot.addEventListener('scroll', $scrollHandler)
    $scrollHandler(footerNode, codeNode)
  }, 400);
}

function scrollHandler(footerNode, codeNode) {
  const { top, bottom, left, right } = codeNode.getBoundingClientRect()
  const offsetWidth = codeNode.offsetWidth
  const isFixed = bottom > document.documentElement.clientHeight &&
    top + 40 <= document.documentElement.clientHeight;
    if (isFixed) {
      footerNode.classList.add(CLASS_FOOTER_FIX)
      footerNode.style.left = `${ left }px`
      footerNode.style.width = `${ offsetWidth }px`
    } else {
      footerNode.classList.remove(CLASS_FOOTER_FIX)
      footerNode.style.left = '0'
      footerNode.style.width = 'auto'
...
function removeScrollHandler($scrollHandler) {
    scrollRoot && scrollRoot.removeEventListener('scroll', $scrollHandler)
}
